<x-admin-layout :title="$title">
    <div class="w-full p-2">
        <h1 class="text-2xl font-bold mb-4">Karyawaan : Create</h1>

        <!-- Card Container -->
        <div class="bg-white h-auto w-full shadow-md rounded-lg p-2">
            <form id="users-form" action="{{ route('pemilik-store-karyawan') }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Set nilai awal sebagai "Nonaktif"
            statusInput.value = "Nonaktif";

            // Event listener untuk ketika toggle diklik
            toggle.addEventListener('change', function() {
                if (toggle.checked) {
                    // Jika toggle dicentang
                    statusInput.value = "Bekerja";
                } else {
                    // Jika toggle tidak dicentang
                    statusInput.value = "Nonaktif";
                }
            });
        });
