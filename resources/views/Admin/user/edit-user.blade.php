<x-admin-layout :title="$title">
    <div class="w-full p-2">
        ...
        <div class="bg-white h-auto w-full shadow-md rounded-lg p-2">
            <form id="users-form" action="{{ route('admin-update-user', $user->uid) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                        ...
                        <div class="flex justify-center gap-4 flex-col lg:flex-row items-center">
                            <div>
                                ...
                                <div class="flex items-center justify-center w-64 h-64">
                                    <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                                        ...
                                        <img class="w-full h-full rounded-md" id="image-update" src="{{ Storage::url('assets/' . $user->foto) }}" alt="">
                                    </label>
                                </div>
                            ...
                            <div class="w-full">
                                <div class="mb-4 flex gap-4 flex-col lg:flex-row">
                                    ...
                                    <div class="w-full">
                                        <label class="block text-sm font-medium text-gray-700" for="role">Role User</label>
                                        <input type="text" value="{{ $user->role }}" id="role" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" name="role" readonly disabled>
                                    </div>
                                </div>
                    ...
                    <div id="info-karyawan" class="hidden mt-4">
                        ...
                        <label class="inline-flex items-center me-5 cursor-pointer">
                            <input type="checkbox" id="toggle" class="sr-only peer"
                                ...
                                onclick="toggleOutletSelection(this)">
                            <div class="relative w-11 h-6 bg-gray-200 rounded-full peer dark:bg-gray-700 peer-focus:ring-4 peer-focus:ring-green-300 dark:peer-focus:ring-green-800 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-green-600"></div>
                        ...
                        <div id="outlet-selection" class="mt-4 {{ $user->status == 'Bekerja' ? '' : 'hidden' }}">
                            ...
                            <div class="w-full mb-2">
                                <input type="text" id="simple-search" onkeyup="searchOutlet()" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search Outlet..." />
                            </div>
                            <ul id="outlet-list" class="grid w-full gap-6 md:grid-cols-2">
                                ...
                                @empty
                                <li>
                                    <label for="" class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 dark:peer-checked:text-blue-500 peer-checked:border-blue-600 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
                                        Tidak ada Outlet
                                    </label>
                                </li>
                                @endforelse
    ...
    <script>
        ...
        function toggleOutletSelection(checkbox) {
            const outletSelection = document.getElementById('outlet-selection');
            // Tampilkan daftar outlet jika checkbox dicentang, sembunyikan jika tidak
            if (checkbox.checked) {
                ...
                document.getElementById('id_outlet').value = null;
            }
        }
       ...
       function toggleOutlets() {
            ...
            const outletSelection = document.getElementById('outlet-selection');
            const outletInputs = document.querySelectorAll('input[name="id_outlet"]');

            // Show or hide the outlet selection based on checkbox status
            outletSelection.classList.toggle('hidden', !checkbox.checked);
            
            // Set status and outlet logic based on checkbox
            if (!checkbox.checked) {
                statusInput.value = 'Berhenti';
                // Set id_outlet to NULL (uncheck all outlets)
                ...
                const selectedOutlet = Array.from(outletInputs).find(input => input.checked);
                if (!selectedOutlet && outletInputs.length > 0) {
                    outletInputs[0].checked = true;
                }
            }
        }

        // Initialize outlet selection visibility on page load
        document.addEventListener('DOMContentLoaded', function () {
            const checkbox = document.getElementById('toggle');
            toggleOutlets(); // Set initial outlet selection visibility
            checkbox.addEventListener('change', toggleOutlets); // Add event listener for checkbox changes
        });
        ...
        function searchOutlet() {
            ...
            const filter = input.value.toLowerCase(); // Mengubah input ke huruf kecil
            const outletList = document.getElementById('outlet-list');
            const outlets = outletList.getElementsByTagName('li'); // Mendapatkan semua elemen <li>

            // Loop untuk menyembunyikan atau menampilkan outlet berdasarkan pencarian
            for (let i = 0; i < outlets.length; i++) {
                const outletLabel = outlets[i].getElementsByTagName('label')[0];
                const outletName = outletLabel.getElementsByClassName('text-lg font-semibold')[0].innerText; // Mengambil nama outlet

                // Memeriksa apakah nama outlet mengandung input pencarian
                if (outletName.toLowerCase().indexOf(filter) > -1) {
                    outlets[i].style.display = ""; // Menampilkan outlet yang cocok
                } else {
                    outlets[i].style.display = "none"; // Menyembunyikan outlet yang tidak cocok
                }
            }
        }
