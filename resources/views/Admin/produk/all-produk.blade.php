<x-admin-layout :title="$title">
    <div class="w-full">
        <div class="flex justify-between flex-col md:flex-row items-center">
            <h1 class="text-3xl font-bold">Daftar Semua Produk</h1>
            <div class="flex flex-col md:flex-row gap-3 items-start w-full md:w-auto mt-4">
                <div class="flex flex-col md:flex-row w-full">
                    <form method="GET" action="{{ route('admin-all-produk') }}" class="flex items-center flex-col md:flex-row w-full gap-2">
                        <!-- Hidden Inputs -->
                        ...
                        <select name="id_outlet" 
                            ...
                            @foreach ($outlets as $outlet)
                                <option value="{{ $outlet->id }}" 
                                        {{ request('id') == $outlet->id || $outlet->id == 'YOUR_DEFAULT_ID' ? 'selected' : '' }}>
                                    {{ $outlet->nama_outlet }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="relative flex items-center justify-center space-x-2 text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900 w-fit">
                                <!-- Button for 'Ubah' -->
                                <a href="{{ route('admin-edit-produk-outlet', ['uid' => $produk->Outlet->uid, 'sku' => $produk->sku]) }}" class="focus:outline-none">
                                    Ubah
                                </a>
