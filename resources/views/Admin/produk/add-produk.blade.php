<x-admin-layout>
    <div class="w-full p-6">
        ...
        <form id="outlet-form" action="{{ route('produks.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <!-- Card Container -->
                    ...
                    <div class="flex w-full gap-4">
                        ...
                        <div class="w-full">
                            ...
                            <div class="mb-4 w-full">
                                ...
                                <input type="text" id="nama_produk" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" name="nama_produk">
                            </div>

                            <div class="flex gap-4">
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-700" for="sku">SKU</label>
                                    <input type="text" id="sku" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" name="sku">
                                </div>
            ...
            <div class="bg-white h-auto w-full shadow-md rounded-lg p-6 flex mt-4 ">
                <div class="w-1/2">
                    ...
                    <div class="flex w-full pr-7 gap-3 mt-4">
                        ...
                        <div class="w-full md:w-1/2 mb-4">
                            ...
                            <input type="text" id="stok_minimum" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" name="stok_minimum">
                        </div>
                    </div>
                    <div class="w-full md:w-1/2 mb-4 pr-7">
                        <label class="block text-sm font-medium text-gray-700" for="harga_jual">Harga Jual</label>
                        <input type="text" id="harga_jual" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" name="harga_jual">
                    </div>
                    <div class="w-full md:w-1/2 mb-4 pr-7">
                ...
                <div class="w-1/2">
                    ...
                    <!-- Checkbox untuk "Tersedia di semua Outlet" -->
                    <div id="outlet-section">
                        <label class="flex items-center me-5 cursor-pointer my-4">
                            <input type="checkbox" id="all-outlet-toggle" class="sr-only peer" name="id_outlet"/>
                            <div class="relative w-11 h-6 bg-gray-200 rounded-full peer dark:bg-gray-700 peer-focus:ring-4 peer-focus:ring-green-300 dark:peer-focus:ring-green-800 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-green-600"></div>
                            <label for="all-outlet-toggle" class="ms-2 text-xl font-semibold text-gray-900 dark:text-gray-300">Tersedia di semua Outlet</label>
                        </label>

                        <!-- Modal toggle button -->
                        <button id="choose-outlet-btn" data-modal-target="outlet-modal" data-modal-toggle="outlet-modal" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                            Pilih Outlet
                        </button>
                    </div>
                </div>
            ...
            <div class="bg-white h-auto w-full shadow-md rounded-lg p-6 mt-4">
                ...
                <textarea id="wysiwyg-editor" name="catatan" class="w-full h-60 border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" ></textarea>
            </div>
            ...
            <!-- Modal Outlet -->
            <div id="outlet-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-2xl max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                            ...
                            <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="outlet-modal">
                            ...
                            <div class="w-full rounded-md px-4">
                                @forelse ($Outlet as $outlet)
                                    <div class="flex items-center mb-4">
                                        <input id="outlet-checkbox-{{ $outlet->id }}" type="checkbox" name="id_outlet[]" value="{{ $outlet->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                        <label for="outlet-checkbox-{{ $outlet->id }}" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $outlet->nama_outlet }}</label>
                                    </div>
                                @empty
                                    <div class="flex items-center mb-4">
                                        <label for="" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Tidak ada Outlet</label>
                                    </div>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                            <button data-modal-hide="outlet-modal" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Terapkan</button>
                            <button data-modal-hide="outlet-modal" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Batal</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        const allOutletToggle = document.getElementById('all-outlet-toggle');
        const outletCheckboxes = document.querySelectorAll('input[name="id_outlet[]"]');
        const chooseOutletBtn = document.getElementById('choose-outlet-btn');

        // Handle "Tersedia di semua Outlet" checkbox
        allOutletToggle.addEventListener('change', function() {
            if (this.checked) {
                // Select all checkboxes
                outletCheckboxes.forEach(checkbox => {
                    checkbox.checked = true;
                });
                chooseOutletBtn.style.display = 'none';
            } else {
                // Deselect all checkboxes
                outletCheckboxes.forEach(checkbox => {
                    checkbox.checked = false;
                });
                chooseOutletBtn.style.display = 'block';
            }
        });
        
        // Toggle outlet section based on POS toggle
