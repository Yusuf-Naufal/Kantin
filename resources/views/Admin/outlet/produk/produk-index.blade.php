<x-admin-layout :title="$title">
    <div class="w-full">
        <div class="flex justify-between mt-1">
            <h1 class="text-3xl font-bold">Daftar Produk : {{ $outlet->nama_outlet }}</h1>
        </div>

        <div class="flex justify-between flex-col md:flex-row items-center mt-2">

            <a href="{{ route('admin-add-produk-outlet', $outlet->uid) }}" type="button" 
            ...
            <div class="flex flex-col md:flex-row gap-3 items-start w-full md:w-auto mt-4">
                <div class="flex flex-col md:flex-row w-full">
                    <form method="GET" action="{{ route('admin-all-produk-outlet', $outlet->uid) }}" class="flex items-center flex-col md:flex-row w-full gap-2">
                        <!-- Hidden Inputs -->
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <a target="_blank" class="w-4 h-4">
                                ...
                                @else
                                    <img class="object-contain w-20 h-20 sm:w-16 sm:h-16 md:w-20 md:h-20 min-w-full rounded-md" src="{{ asset('assets/produk.png') }}" alt="Produk">
                                @endif
                            </a>
                        </th>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="relative flex items-center justify-center space-x-2 text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900 w-fit">
                                <!-- Button for 'Ubah' -->
                                <a href="{{ route('admin-edit-produk-outlet', ['uid' => $outlet->uid, 'sku' => $produk->sku]) }}" class="focus:outline-none">
                                    Ubah
                                </a>
                                ...
                                <div id="dropdown-{{ $index }}" class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                        @if($produk->status === 'Habis')
                                            <li>
                                               <form action="{{ route('admin-status-produk-outlet', ['uid' => $outlet->uid, 'sku' => $produk->sku]) }}" method="POST" class="block">
                                                    @csrf
                                                    @method('PATCH')
                                        ...
                                        @endif
                                        <li>
                                            <form id="delete-form-{{ $produk->id }}" action="{{ route('admin-destroy-produk-outlet', ['uid' => $outlet->uid, 'sku' => $produk->sku]) }}" method="POST" class="block">
                                                @csrf
                                                @method('DELETE')
